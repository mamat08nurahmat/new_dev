...
//		$query = $this->db->query("select concat(iso3, concat(':', iso3)) as name from $table_name where iso3 is not null");
			$query = $this->db->query("select concat(FirstName, concat(':', FirstName)) as FirstName from $table_name where FirstName is not null");
		//Get contents
...
//		$this->db->select('id,iso,name,printable_name,iso3,numcode,created_date')->from($table_name);
			$this->db->select('ID,FirtName,LastName,Age')->from($table_name);
	...
	public function get_all_countries() 
	{
		...
		//Build contents query
//		$this->db->select('id,iso,name,printable_name,iso3,numcode,created_date')->from($table_name);
			$this->db->select('ID,FirstName,LastName,Age')->from($table_name);
		$this->CI->flexigrid->build_query(FALSE);
