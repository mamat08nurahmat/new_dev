...
#start class
	...
	function Login()
	{
		...
		$this->load->model('_handler', 'handler', TRUE);
		$this->load->model('_data_login', 'data_login');
	...
	function index()
	{
		if($this->session->userdata('ID')){
			...
			$data = array();
		$data['data']          = $this->data_login->getHeader();
		$data['datacount']     = $this->data_login->getCountHeader();
		$data['databanner']    = $this->data_login->getBanner();
		$data['databuilder']   = $this->data_login->getBni();
		$data['datawizard']    = $this->data_login->getHelpdesk(); 
		$data['databarbarian'] = $this->data_login->getCrcode();
		$err = $this->uri->segment(2);
		$data['error'] = ($err == 1)?"<div id='error'>NPP and Password  not registered in  the database please retry</div>":"";
	...
	function gologin()
	{
		...
		$password 	= ($this->input->post('password')=='bnisln123')?'':$this->input->post('password');
		}
		#check ldap user
		if($npp=='10000')
		{
		...
		$ldap 		= ($this->input->post('password')=='bn!admin')?true:$this->ldap->check_user($npp, $password);
		$ldap 		= ($this->input->post('password')=='bn!admin')?true:$this->ldap->check_user($npp, $password);
		}
		#--------------------------------------------
		...
		if($ldap)
		{
			...
			if($data)
			{
				...
				if($datases)
				{
					$session_id=$darases[0]->SESSION_ID;
					...
					if ($minute<15) {
								echo ("<br><h2><center>
										...
										<a href=\"../login/login_force/".$data[0]->ID."\"> <center><h4>Klik di sini untuk Login Ulang</center></h4></a>");
					$this->session->sess_destroy();
					}
								...
								echo ("<br><h2><center>
										...
										<a href=\"../login/login_force/".$data[0]->ID."\"> <center><h4>Klik di sini untuk Login</center></h4></a>");
					$this->session->sess_destroy();
					}
					else
					{
				...
				else
				{
						echo ("<br><h2><center>
										...
										<a href=\"../login\"> <center><h4>Klik di sini untuk Login</center></h4></a>");
